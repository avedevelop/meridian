...
export function StatusBar() {
  ...
  return (
    ...
    >
      ...
      {cursorPos && (
        <span>{t('statusBar.line', { line: cursorPos.line, col: cursorPos.col })}</span>
      )}
      <span>{t('statusBar.markdown')}</span>
      {activeTab?.isDirty && (
        <span style={{ color: 'var(--accent-color)' }}>{t('statusBar.unsavedChanges')}</span>
      )}
      ...
      {gitBackupEnabled && gitState && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 12 }}>
          ...
          {!gitState.isRepo ? (
            ...
            <>
              {syncState === 'syncing' && (
                <span style={{ color: 'var(--accent-color)' }}>{t('statusBar.syncing')}</span>
              )}
              {syncState === 'success' && (
                <span style={{ color: '#10b981' }}>{t('statusBar.synced')}</span>
              )}
              {syncState === 'error' && (
                <span
              ...
              {syncState === 'idle' && (
                <span
                  style={{
                    opacity: gitState.clean ? 0.5 : 0.9,
                    color: gitState.clean ? 'inherit' : 'var(--accent-color)'
                  }}
                >
                  {gitState.clean
                    ? t('statusBar.upToDate')
                    : t('statusBar.unsaved', { count: gitState.changesCount })}
                </span>
              )}
