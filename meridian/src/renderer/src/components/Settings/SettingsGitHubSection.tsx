...
export function SettingsGitHubSection({ isOpen }: SettingsGitHubSectionProps) {
  ...
  return (
    <div
      style={{ background: '#161616', border: '1px solid #252525', borderRadius: 10, padding: 20 }}
    >
      <div style={{ fontSize: 13, fontWeight: 600, color: '#fff', marginBottom: 4 }}>
        {t('github.account')}
      </div>
      <div style={{ fontSize: 12, color: '#777', marginBottom: 16 }}>{t('github.accountDesc')}</div>
      {ghConnected ? (
        <div
          style={{
            display: 'flex',
        ...
        <div style={{ textAlign: 'center', padding: '16px 0' }}>
          <div style={{ fontSize: 12, color: '#aaa', marginBottom: 12 }}>
            {t('github.enterCodeAt')}{' '}
            <span style={{ color: 'var(--accent-color)' }}>github.com/login/device</span>
          </div>
          <div
            style={{
        ...
        <div>
          ...
          {ghError && (
            <div style={{ marginTop: 10, fontSize: 12, color: '#f87171' }}>{ghError}</div>
          )}
        </div>
      )}
    </div>
